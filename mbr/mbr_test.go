...
func Test(t *testing.T) {
	bsdata := testBS
	bs, err := ToBootSector(bsdata[:])
	if err != nil {
		t.Fatal(err)
	}
	for i := 0; i < 4; i++ {
		pte := bs.PartitionTable(i)
		t.Logf("pte[%d].Attributes() = %v", i, pte.Attributes())
		t.Logf("pte[%d].PartitionType() = %v", i, pte.PartitionType())
		t.Logf("pte[%d].StartSector() = %v", i, pte.StartSector())
		t.Logf("pte[%d].NumSectors() = %v", i, pte.NumberOfSectors())
		t.Log(pte.CHSStart())
		t.Log(pte.CHSLast())
	}
}
